...
//myso, odstranujeme, bo to dako koliduje s kinect VS projektom.. #define STRICT
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <process.h>
                             ...
                             char ByteSize , bool modem_events)
{
    ...
    if (port_arg!=0)
    {
        ...
        */
        ...
        /* -------------------------------------------------------------------- */
        //myso hack pre UNICODE problem
		// http://stackoverflow.com/questions/32229966/best-way-to-fill-wstring-with-const-char
		std::vector<char> v(port, port + 10);
		std::wstring wstr(v.begin(), v.end());

		
		serial_handle    = CreateFile(wstr.c_str()/*port*/, GENERIC_READ | GENERIC_WRITE,
...
/* ---------------------           setManager     --------------------- */
/* -------------------------------------------------------------------- */
void         Tserial_event::setManager(type_myCallBack manager_arg, void* insertParam)
{
        manager = manager_arg;
		myInsertParam=insertParam;
}
/* -------------------------------------------------------------------- */
/* ---------------------           setRxSize      --------------------- */
/* -------------------------------------------------------------------- */
...
void Tserial_event::OnEvent (unsigned long events)
{
    ...
    if ((events & EV_RING)!=0)
    {
        ...
        {
            if (manager!=0)
                manager((uint32) this, SERIAL_RING,myInsertParam);
        }
    }
    ...
    if ((events & EV_RLSD)!=0)
    {
        ...
        {
            if (manager!=0)
                manager((uint32) this, SERIAL_CD_ON,myInsertParam);
        }
        else
        {
            if (manager!=0)
                manager((uint32) this, SERIAL_CD_OFF,myInsertParam);
        }
    }
}
/* -------------------------------------------------------------------- */
...
void Tserial_event::run(void)
{
    ...
    if (manager!=0)
        manager((uint32) this, SERIAL_CONNECTED,myInsertParam);
            ...
            switch(status)
            {
                ...
                case SIG_READER:
                        ...
                        &result_nbr, FALSE))
                    {
                        ...
                        if ((result_nbr!=0) &&(manager!=0))
                            manager((uint32) this, SERIAL_DATA_ARRIVAL,myInsertParam);
                ...
                case SIG_WRITER:
                        ...
                        &result_nbr, FALSE))
                    {
                        ...
                        if (manager!=0)
                            manager((uint32) this, SERIAL_DATA_SENT,myInsertParam);
                    }
                    else
                    {
                        // GetOverlapped didn't succeed !
    ...
    if (manager!=0)
        manager((uint32) this, SERIAL_DISCONNECTED,myInsertParam);
}
/* -------------------------------------------------------------------- */
/* -------------------------------------------------------------------- */
